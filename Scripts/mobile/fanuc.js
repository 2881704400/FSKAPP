...
function fanuc() {
	...
	$("#fanucCenterTitleID").html(chatTitleName);
	initConnectService();
}
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(datas != null && datas != "" && datas != "false") {
				getRealTimeDataByID_Fanuc();
				//setInterval(getRealTimeDataByID_Fanuc, 3000);
			}
		},
...
function getRealTimeDataByID_Fanuc() {
	$.ajax({
		type: "POST",
			...
			$(data).find('string').each(function() {
				...
				if(result&&result[0].m_YCValue != "***") {
					for(var i = 0; i < result.length; i++) {
						if(result[i].m_iYCNo == 856) {
							$(".fanuc_RunState").parent().find('span').html(result[i].m_YCValue);
						}
						if(result[i].m_iYCNo == 490) {
							$(".fanuc_StatusState").parent().find('span').html(result[i].m_YCValue);
						}
						if(result[i].m_iYCNo == 857) {
							$(".fanuc_PauseState").parent().find('span').html(result[i].m_YCValue);
						}
						if(result[i].m_iYCNo == 710) {
							$(".fanuc_Speed").parent().find('span').html(result[i].m_YCValue);
						}
					}
				}
			});
